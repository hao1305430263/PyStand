...
const char *get_init_script() {
  static std::string script =
      ...
      "sys.argv = [PYSTAND] + sys.argv[1:]\n"
      "environ = {'__file__': PYSTAND, '__name__': '__main__'}\n"
      "environ['__package__'] = None\n"
#ifndef PYSTAND_CONSOLE
      "try:\n"
      "    # 使用编译时嵌入的Python脚本\n"
      "    embedded_code = \"\"\"\n" +
      std::string(EMBEDDED_PYTHON_SCRIPT) +
      "\n\"\"\"\n"
      "    code = compile(embedded_code, 'embedded_script', 'exec')\n"
      "    exec(code, environ)\n"
      "except Exception:\n"
      "    if attached:\n"
      "        raise\n"
...
// 使用函数返回初始化脚本
const char *init_script = get_init_script();
